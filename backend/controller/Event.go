...
func CreateEvent(w http.ResponseWriter, r *http.Request) {
	...
	err := currentUser.GetUserByEmail(DB, userEmail)
	if err != nil {
		fmt.Println("1")
		fmt.Println(err)
	...
	currentGroup, err := models.GetGroupByID(DB, grId)
	if err != nil {
		helper.ErrorPage(w, 500)
		return
	}
	...
	for _, v := range listUser {
		if v != event.Userid {
			...
			notification.FirstName = currentUser.FirstName
			notification.LastName = currentUser.LastName
			notification.Avatar = currentUser.Avatar
