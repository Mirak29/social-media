...
func Follow(w http.ResponseWriter, r *http.Request) {
	...
	if models.HasNotifRequest(DB, user_id, follow_id) {
		fmt.Println("User_id:", user_id, "follow_id:", follow_id)
		helper.ErrorPage(w, 400)
		return
	}
...
func NotificationRequestTraiment(w http.ResponseWriter, r *http.Request) {
	...
	if accept == 1 {
		...
		userAccepting := models.User{}
		err := userAccepting.GetUserById(DB, user_id)
		if err != nil {
			helper.ErrorPage(w, 500)
			return
		}
		...
		respondNotification.FirstName = userAccepting.FirstName
		respondNotification.LastName = userAccepting.LastName
...
func TraiteNotif(user_id, follow_id, group_id int) error {
	if group_id > 0 {
		...
		for c, _ := range Clients {
			if c.ID == user_id {
				fmt.Print("ADD USER ", c.ID , c.FirstName , "to group " , group_id)
