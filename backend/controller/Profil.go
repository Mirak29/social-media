...
func GetContacts(w http.ResponseWriter, r *http.Request) {
	...
	groups, errg := models.GetMemberGroupsByUserID(DB, user_id)
	if errg != nil {
		helper.ErrorPage(w, 500)
		return
	}
	
	err := helper.WriteJSON(w, http.StatusOK, map[string]interface{}{"success": true,"contacts" : contacts, "groups": groups}, nil)
	if err!=nil {
		fmt.Println(err)
		helper.ErrorPage(w, 400)
		return
